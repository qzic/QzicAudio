...
public class AudioSampleReader {
    ...
    // Get the interleaved decoded samples for all channels, from sample
    ...
    public void getInterleavedSamples(long begin, long end,
        ...
        // System.out.println("inBuffer.length = " + inBuffer.length + ", samples.length = " + samples.length);
...
/*        
        System.out.println("interleavedSamples.length = " + interleavedSamples.length +
        		", channelSamples.length = " + channelSamples.length);
        */
    ...
    // Extract stereo samples from interleavedSamples and
    // copy them into stereoSamples
    public void extractStereoSamples(double[] interleavedSamples, double[] stereoSamples) {
        int nbChannels = format.getChannels();
        int k=0;
        for (int i = 0; i < interleavedSamples.length/nbChannels; i++) {
        	stereoSamples[k++] = interleavedSamples[nbChannels*i];
        	stereoSamples[k++] = interleavedSamples[nbChannels*i+1];
        }
    }
    ...
    // Convenience method. Extract left and right channels for common stereo
    // files. leftSamples and rightSamples must be of size getSampleCount()
    public void putStereoSamples(double[] leftSamples, double[] rightSamples, double[] interleavedSamples)
            throws IOException {
        for (int i = 0; i < leftSamples.length; i++) {
            interleavedSamples[2*i] 	= leftSamples[i];
            interleavedSamples[2*i+1]	= rightSamples[i];
        }        
    }
    ...
    private void decodeBytes(byte[] audioBytes, double[] audioSamples) {
        ...
        for (int i = 0; i < audioSamples.length; i++) {
            ...
            int ival = (int)(sampleBytes[0]) << 8;
            for (int j = 1; j < sampleSizeInBytes; j++) {
                ival |= (int) (sampleBytes[j]) & 0xFF;
    ...
    // Decode audioSamples into bytes of audioBytes
    public void encodeBytes(int bytesPerSample, double[] audioSamples, byte[] audioBytes) {
        double ratio = Math.pow(2., bytesPerSample*(8.0-1) - 1);
        byte[] sampleBytes = new byte[bytesPerSample];
        int k = 0; // index in audioBytes
        
        for (int i = 0; i < audioSamples.length; i++) {
            double val = audioSamples[i];
            int ival = (int) (val * ratio);
            
            // get bytes from integer value
            for (int j = bytesPerSample - 1; j >= 0; j--) {
                sampleBytes[j] = (byte) (ival & 0xFF);
                ival >>= 8;
            }
            
            // distribute sample byte in big-endian order
            if (format.isBigEndian()) {
                // bytes start with MSB
                for (int j = 0; j < bytesPerSample; j++) {
                    audioBytes[k++] = sampleBytes[j];
                }
            } else {
                // bytes start with LSB
                for (int j = bytesPerSample - 1; j >= 0; j--) {
